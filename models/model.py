...
from .Modules import TeacherNet, StudentNet, FusionNet, TransLayer, Pooler, UniTeacherNet
...
def define_net(args, path_only = False, omic_only = False):
    ...
    if args.mode == "path":
        ...
        net = CMTA(args=args)
    elif args.mode == "pathomic_fg":
        print("creating pathomic_fg model")
        net = PathomicNet_FG(args=args)
    elif args.mode == "pathomic_ensemble":
        print("creating pathomic_ensemble model")
        net = PathomicNet_Ensemble(args=args)
    elif args.mode == "deformpathomic":
        print("creating deformpathomic model")
        net = DeformPathomicNet(args=args)
    elif args.mode == "multiscale":
        print("creating multiscale model")
        net = MultiScaleNet(args=args)
    elif args.mode == 'teacher':
        print('training teacher model')
        net = TeachersNet(args=args)
    elif args.mode == 'uniteacher':
        print('training uniteacher model')
        net = UniTeachersNet(args=args)
    elif args.mode == 'student':
        print('training student model')
        net = StudentsNet(args=args)
    elif args.mode == 'distillation':
        print('creating distillation')
        net = StudentsNet(args=args)
        teacher_net = TeachersNet(args=args)
        return net, teacher_net
    else:
...
def define_scheduler(args, optimizer, step_per_epoch=0):
    if args.lr_policy == 'linear':
        ...
        # scheduler = lr_scheduler.CosineAnnealingLR(optimizer, T_max=5, eta_min=1.0e-10)
        # scheduler = lr_scheduler.CosineAnnealingWarmRestarts(optimizer, T_0=10, T_mult=2, eta_min=1.0e-10)
    elif args.lr_policy == 'cosinewithwarmup':
...
class TeacherStudentNet(nn.Module):
    def __init__(self, args):
        ...
        self.omic_net_immune = MaxNet(input_dim=args.input_size_omic_immune, omic_dim=args.omic_dim, return_grad = args.return_grad,
                               dropout_rate=args.dropout_rate, label_dim=args.label_dim, init_max=init_max)
        self.student_net = StudentNet(args)
        self.teacher_net = TeacherNet(args)

    def forward(self, x_path, x_omic_tumor, x_omic_immune):

        omic_vec_tumor, hazard_omic, omic_grads = self.omic_net_tumor(x_omic=x_omic_tumor) #input: B,59
        omic_vec_immune, hazard_omic, omic_grads = self.omic_net_immune(x_omic=x_omic_immune)

        logits_stu, hazards_stu, S_stu, risk_stu, feature_stu, att_stu = self.student_net(path=x_path) #[B, 128]
        # logits_student, pathpath_vec, pathpath_att = self.student_net(path=x_path)

        logits_tea, hazards_tea, S_tea, risk_tea, feature1_tea, feature2_tea, att_omic1_tea, att_omic2_tea = self.teacher_net(path=x_path, omic_list=[omic_vec_tumor, omic_vec_immune]) #[B, 128]
        # logits_teacher, omic1path_vec, omic2path_vec, omic1path_att, omic2path_att = self.teacher_net(path=x_path, omic_list=[omic_vec_tumor, omic_vec_immune])

        # print('pathpath_vec.shape:', pathpath_vec.shape) #[B, 128]  
        # print('omic1path_vec.shape:', omic1path_vec.shape) #[B, 128] 
        # print('omic2path_vec.shape:', omic2path_vec.shape) #[B, 128]   
        # features = torch.cat((pathpath_vec, omicpath_vec), 1) # [B, 128*2] for others
        
        # return logits_student, logits_teacher, pathpath_vec, omic1path_vec, omic2path_vec, pathpath_att, omic1path_att, omic2path_att
        return logits_stu, hazards_stu, S_stu, risk_stu, feature_stu, att_stu, logits_tea, hazards_tea, S_tea, risk_tea, feature1_tea, feature2_tea, att_omic1_tea, att_omic2_tea
...
class MultiScaleNet(nn.Module):
    def __init__(self, args):
        ...
        # self.path_net = TransMIL(args)
        self.args = args
        self.scale10_net = TeacherStudentNet(args)
        self.scale20_net = TeacherStudentNet(args)

        self.cls_token = nn.Parameter(torch.randn(1, 1, args.mmhid))

        self.fusion = FusionNet(feature_dim=args.mmhid*2)
        
        self.combination_type = args.combination_type_teas

    def combine_logits(self, logits1, logits2):
        if self.combination_type == 'average':
            ...
            # Weighted average
            final_logits = self.weights[0] * logits1 + self.weights[1] * logits2
            
        elif self.combination_type == 'softmax_average':
            # Softmax then average
            prob1 = torch.softmax(logits1, dim=1)
            prob2 = torch.softmax(logits2, dim=1)
            # Convert back to logits space using log
            final_logits = torch.log((prob1 + prob2) / 2 + 1e-8)  # add small epsilon to avoid log(0)
            
        elif self.combination_type == 'max_confidence':
            prob1 = torch.softmax(logits1, dim=1)
            prob2 = torch.softmax(logits2, dim=1)
            conf1, _ = torch.max(prob1, dim=1)
            conf2, _ = torch.max(prob2, dim=1)
            mask = (conf1.unsqueeze(1) > conf2.unsqueeze(1))
            final_logits = torch.where(mask, logits1, logits2)

        return final_logits

    def forward(self, **kwargs):
        logits_stu10, hazards_stu10, S_stu10, risk_stu10, feature_stu10, att_stu10, logits_tea10, hazards_tea10, S_tea10, risk_tea10, feature1_tea10, feature2_tea10, att1_tea10, att2_tea10 = self.scale10_net(x_path=kwargs['x_path10'], x_omic_tumor=kwargs['x_omic_tumor'], x_omic_immune=kwargs['x_omic_immune'])
        logits_stu20, hazards_stu20, S_stu20, risk_stu20, feature_stu20, att_stu20, logits_tea20, hazards_tea20, S_tea20, risk_tea20, feature1_tea20, feature2_tea20, att1_tea20, att2_tea20 = self.scale20_net(x_path=kwargs['x_path20'], x_omic_tumor=kwargs['x_omic_tumor'], x_omic_immune=kwargs['x_omic_immune'])

        feature_dict = {'feature1_tea10': feature1_tea10, 
                         'feature2_tea10': feature2_tea10,
                         'feature1_tea20': feature1_tea20, 
                         'feature2_tea20': feature2_tea20,
                         'feature_stu10': feature_stu10,
                         'feature_stu20': feature_stu20,
                         }

        att_dict = {'att1_tea10':att1_tea10,
                    'att2_tea10':att2_tea10,
                    'att1_tea20':att1_tea20,
                    'att2_tea20':att2_tea20,
                    'att_stu10':att_stu10,
                    'att_stu20':att_stu20,
                    }

        # Combine predictions to get final logits 
        logits_stus = self.combine_logits(logits_stu10, logits_stu20)

        logits_dict = {'logits_tea10': logits_tea10, 
                        'logits_tea20': logits_tea20,
                        'logits_stu10': logits_stu10, 
                        'logits_stu20': logits_stu20,
                        'logits_stus': logits_stus,
                        }

        hazards_stus = torch.sigmoid(logits_stus)

        hazards_dict = {'hazards_tea10': hazards_tea10, 
                         'hazards_tea20': hazards_tea20,
                         'hazards_stu10': hazards_stu10, 
                         'hazards_stu20': hazards_stu20,
                         'hazards_stus': hazards_stus,
                        }

        S_stus = torch.cumprod(1 - hazards_stus, dim=1)
        
        S_dict = {'S_tea10': S_tea10, 
                  'S_tea20': S_tea20,
                  'S_stu10': S_stu10, 
                  'S_stu20': S_stu20,
                  'S_stus': S_stus,
        ...
        risk_stus = -torch.sum(S_stus, dim=1)

        risk_dict = {'risk_tea10': risk_tea10, 
                     'risk_tea20': risk_tea20,
                     'risk_stu10': risk_stu10, 
                     'risk_stu20': risk_stu20,
                     'risk_stus': risk_stus,
                     }    
        
        return feature_dict, att_dict, logits_dict, hazards_dict, S_dict, risk_dict

# Seperate Training (Distillation)
class UniTeachersNet(nn.Module):
    def __init__(self, args):
        super(UniTeachersNet, self).__init__()
        init_max = True if args.init_type == "max" else False
        ...
        self.omic_net_immune = MaxNet(input_dim=args.input_size_omic_immune, omic_dim=args.omic_dim, return_grad = args.return_grad,
                               dropout_rate=args.dropout_rate, label_dim=args.label_dim, init_max=init_max)
        self.teacher10_net = UniTeacherNet(args)
        self.teacher20_net = UniTeacherNet(args)
    ...
    def forward(self, x_path10, x_path20, x_omic_tumor, x_omic_immune):

        omic_vec_tumor, hazard_omic, omic_grads = self.omic_net_tumor(x_omic=x_omic_tumor) #input: B,59
        omic_vec_immune, hazard_omic, omic_grads = self.omic_net_immune(x_omic=x_omic_immune)

        logits_tea10, hazards_tea10, S_tea10, risk_tea10, feature1_tea10, feature2_tea10, att1_tea10, att2_tea10 = self.teacher10_net(path=x_path10, omic_list=[omic_vec_tumor, omic_vec_immune]) #[B, 128]
        logits_tea20, hazards_tea20, S_tea20, risk_tea20, feature1_tea20, feature2_tea20, att1_tea20, att2_tea20 = self.teacher20_net(path=x_path20, omic_list=[omic_vec_tumor, omic_vec_immune])
        # logits_teacher, omic1path_vec, omic2path_vec, omic1path_att, omic2path_att = self.teacher_net(path=x_path, omic_list=[omic_vec_tumor, omic_vec_immune])
        
        # Combine predictions to get final logits 
        logits_teas = self.combine_logits(logits_tea10, logits_tea20)
        logits_dict = {'logits_tea10': logits_tea10,
                       'logits_tea20': logits_tea20,
                       'logits_teas':logits_teas,}
        
        hazards_teas = torch.sigmoid(logits_teas)
        hazards_dict = {'hazards_tea10': hazards_tea10,
                       'hazards_tea20': hazards_tea20,
                       'hazards_teas': hazards_teas,}
        
        S_teas = torch.cumprod(1 - hazards_teas, dim=1)
        S_dict = {'S_tea10': S_tea10,
                  'S_tea20': S_tea20,
                  'S_teas': S_teas,}  

        risk_teas = -torch.sum(S_teas, dim=1)                      
        risk_dict = {'risk_tea10': risk_tea10,
                     'risk_tea20': risk_tea20,
                     'risk_teas': risk_teas,}
...
# Seperate Training (Distillation)
class TeachersNet(nn.Module):
    def __init__(self, args):
        super(TeachersNet, self).__init__()
        init_max = True if args.init_type == "max" else False
        ...
        self.omic_net_immune = MaxNet(input_dim=args.input_size_omic_immune, omic_dim=args.omic_dim, return_grad = args.return_grad,
                               dropout_rate=args.dropout_rate, label_dim=args.label_dim, init_max=init_max)
        self.teacher10_net = TeacherNet(args)
        self.teacher20_net = TeacherNet(args)
...
class StudentsNet(nn.Module):
    def __init__(self, args):
        super(StudentsNet, self).__init__()
        
        # self.path_net = ABMIL(args)
        # self.path_net = TransMIL(args)
        self.args = args
        self.student10_net = StudentNet(args)
        self.student20_net = StudentNet(args)
        self.combination_type = args.combination_type_stus
    #     self._init_params()
    ...
    def forward(self, x_path10, x_path20):

        logits_stu10, hazards_stu10, S_stu10, risk_stu10, feature_stu10, att_stu10 = self.student10_net(path=x_path10) #[B, 128]
        logits_stu20, hazards_stu20, S_stu20, risk_stu20, feature_stu20, att_stu20 = self.student20_net(path=x_path20) #[B, 128]

        # Combine predictions to get final logits 
        logits_stus = self.combine_logits(logits_stu10, logits_stu20)
        logits_dict = {'logits_stu10': logits_stu10,
                       'logits_stu20': logits_stu20,
                       'logits_stus': logits_stus}
        
        hazards_stus = torch.sigmoid(logits_stus)
        hazards_dict = {'hazards_stu10': hazards_stu10,
                       'hazards_stu20': hazards_stu20,
                       'hazards_stus': hazards_stus,}
        
        S_stus = torch.cumprod(1 - hazards_stus, dim=1)
        S_dict = {'S_stu10': S_stu10,
                  'S_stu20': S_stu20,
                  'S_stus': S_stus,}  
          
        risk_stus = -torch.sum(S_stus, dim=1)                 
        risk_dict = {'risk_stu10': risk_stu10,
                     'risk_stu20': risk_stu20,
                     'risk_stus': risk_stus,}
        
        feature_dict = {'feature_stu10': feature_stu10,
                         'feature_stu20': feature_stu20,}
        att_dict = {'att_stu10': att_stu10,
                    'att_stu20': att_stu20,}
...
############################################
class PathomicNet_FG(nn.Module):
    def __init__(self, args):
        super(PathomicNet_FG, self).__init__()
        init_max = True if args.init_type == "max" else False
        self.args = args
        ...
        self.path2omic_proj = nn.Sequential(nn.Linear(args.mmhid, args.mmhid))
        self.ae = nn.Sequential(
            nn.Linear(args.mmhid, 64),
            nn.LayerNorm(64),
            nn.Linear(64, args.mmhid))

    def forward(self, **kwargs):
        path_vec, logits_path, path_grads = self.path_net(kwargs['x_path'])  # (BS,2500,1024), x_path x pathology
        omic_vec, logits_omic, omic_grads = self.omic_net(x_omic=kwargs['x_omic'])

        # print('path_vec.shape:', path_vec.shape) #B,128
        # print('omic_vec.shape:', omic_vec.shape) #B,128

        ## feature generation here!
        # path2omic_vec = self.path2omic_proj(path_vec) #[B, 128]->[B, 128] v2_LM
        path2omic_vec = self.ae(path_vec) #[B, 128]->[B, 128] v2_AE

        if kwargs['is_training']:
            ## features = torch.cat((path_vec, omic_vec), 1) #v1
            features = torch.cat((path_vec, path2omic_vec), 1) #v2
        else:
            features = torch.cat((path_vec, path2omic_vec), 1)

        logits_final = self.classifier(features)

        ### logits for the three branches.
        logits = [logits_path, logits_omic, logits_final]
        return features, path2omic_vec, omic_vec, logits, None, path_grads, omic_grads
...
### Missing Modality: Feature Generation ###
############################################
class PathomicNet_Ensemble(nn.Module):
    def __init__(self, args):
        super(PathomicNet_Ensemble, self).__init__()
        init_max = True if args.init_type == "max" else False
        self.args = args
    ...
    def forward(self, **kwargs):
        ...
        if kwargs['is_training']:
            logits_final = (logits_path + logits_omic) / 2
        else:
            logits_final = logits_path
