...
class DeformCrossAttention2D(nn.Module):
    ...
    def forward(self, x1, x2, return_vgrid = False):
        ...
        # 50,50->int(np.sqrt(x1.shape[2])), int(np.sqrt(x1.shape[2]))
        x1 = x1.view(x1.shape[0], x1.shape[1], int(np.sqrt(x1.shape[2])), int(np.sqrt(x1.shape[2])))
        x2 = x2.view(x2.shape[0], x2.shape[1], int(np.sqrt(x2.shape[2])), int(np.sqrt(x2.shape[2]))) 
        ...
        attn_return = attn
        # print('attn_return:', attn_return) #not same [4, 8, 2500, 144]
        ...
        # print('attn_return.shape:', attn_return.shape)

        # attn_return.shape: torch.Size([4, 8, 2500, 144])
        # vgrid.shape: torch.Size([32, 2, 12, 12])

        # return out # for deformpathomic
        # return out, attn_return, grid, vgrid #torch.Size([2, 12, 12]), torch.Size([8, 2, 12, 12])
        return out, attn_return # for teacher
